...
const FEED = [
  { t: "3분 전",  fandom: "nj" as FandomKey,  gain: "+0.5%" },
  { t: "11분 전", fandom: "ive" as FandomKey, gain: "+0.4%" },
  { t: "26분 전", fandom: "nj" as FandomKey,  gain: "+0.5%" },
];
...
export default function SpotDetail({ params }: PageProps) {
  // Next.js 15 dynamic routing parameter unwrap
  const { id } = use(params);
  
  const [attrib, setAttrib] = useState<FandomKey>("nj"); // 인증 귀속 팬덤
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#FFFFFF", overflow: "hidden", position: "relative" }}>
      ...
      <div className="scroll" style={{ flex: 1, overflowY: "auto" }}>
        ...
        <div style={{ position: "relative", height: 260,
          ...
          <div style={{ position: "absolute", top: 44, left: 14, right: 14, display: "flex",
            ...
            <Link href="/map" style={{ textDecoration: "none" }}>
              <div className="tapzone" style={btnCircle}><ChevronLeft size={20} color={C.txt} /></div>
            </Link>
            <div className="tapzone" style={btnCircle}><Share2 size={17} color={C.txt} /></div>
          </div>
          {/* 심볼 */}
        ...
        <div style={{ padding: "16px 18px 0" }}>
          ...
          <h1 style={{ margin: "6px 0 0", fontSize: 24, fontWeight: 800, letterSpacing: "-0.02em" }}>{SPOT.event} ({id})</h1>
        ...
        <div style={{ margin: "18px 14px 0", background: C.panel, border: `1px solid ${C.line}`, borderRadius: 18, padding: 16 }}>
          ...
          {/* 전황 연결 */}
          <Link href="/war" style={{ textDecoration: "none", width: "100%", display: "block", marginTop: 14 }}>
            <div className="tapzone" style={{ display: "flex", alignItems: "center",
      ...
      <div style={{ flexShrink: 0, borderTop: `1px solid ${C.line}`, background: C.frame, padding: "12px 16px 22px", position: "relative" }}>
        ...
        <Link href="/verify" style={{ textDecoration: "none", width: "100%", display: "block" }}>
          <button className="tapzone" onClick={() => { setPressed(true); }}
            style={{ width: "100%", height: 52, borderRadius: 16, border: "none", cursor: "pointer",
              ...
              boxShadow: `0 10px 26px ${FAND[attrib].color}55` }}>
            <Camera size={19} />
            {pressed ? "카메라 여는 중…" : "영수증 인증하기"}
          </button>
        </Link>
      </div>
    </div>
