...
## 7. 개발 완료 후 에이전트별 교차 검수 파이프라인 (Multi-Agent Verification Pipeline)
모든 피처 및 기능 개발이 완료된 직후, 커밋/푸시 및 사용자 완수 보고 전 다음 **3단계 파이프라인 교차 검수**를 반드시 순차적으로 수행해야 합니다:

1. **1단계: 가온 (개발 에이전트 자체 테스트)**
   - 개발 완료 직후 `npm run lint` 및 `npm run build`를 구동하여 0 경고 / 0 에러 로컬 무결점 빌드 및 기본 동작 자체 검증.
2. **2단계: 다올 (디자인/UX 정밀 검수 필수)**
   - HTML 퍼블리싱 가이드/디자인 스펙 문서(`docs/02_design/`)의 픽셀 레벨 요소(폰트, 패딩, 테두리, 컬러, 반응형 레이아웃)와 구현 화면을 1:1 대조 검수.
3. **3단계: 바른 / 한결 / 여울 (기능 & 비즈니스 로직 & QA 테스트)**
   - 데이터 흐름, 상태 변화, 에지 케이스, API 연동 및 서비스 기획 문서(PRD)와의 비즈니스 로직 정합성 기능 테스트 수행.
