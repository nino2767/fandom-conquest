...
### 팬덤 (Fandom)
- **정의**: 유저가 응원하는 팬덤 IP 및 지도를 물들이는 식별 대표 컬러 토큰.
- **주요 속성**: 팬덤 고유 ID, 정식명, 영문/별칭(Aliases), 시그니처 HEX 컬러, 점령 가중치 배율(`weight_mult`).
- **정책 (3계층 구조)**: **그룹(GROUP), 유닛(UNIT), 개인/솔로/배우(SOLO)** 3계층 트리 매핑 아키텍처 지원.
- **점수 상속 룰 (Upward Roll-up Only)**: 개인/유닛으로 인증 시 **최상위 단체 그룹 점수로 100% 자동 상향 상속**. 단체 그룹으로 인증 시 **하위 개인 멤버로는 하향 분배 안 함**.
- **표기 롤 분담**: 지도 성지 핀 마커는 **개인 최애 시그니처 컬러/엠블럼**, 25구 영토 전체 채색은 **그룹 합산 점수** 기준.
