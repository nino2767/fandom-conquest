...
import { Trophy, Crown, TrendingUp, Users, Flame } from "lucide-react";

/* ============================================================
   IP 팬덤 땅따먹기 — S5 랭킹
   개인(누적 인증·영구) / 팬덤(현재 점유·유동) · 범위 전체·구
   ============================================================ */
...
export default function Ranking() {
  const [mode, setMode] = useState("people"); // people | fandom
  const [scope, setScope] = useState("all");   // all | mapo
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.page, display: "flex", justifyContent: "center",
      ...
      <div style={{ width: 390, maxWidth: "100%", height: 820, background: C.frame, borderRadius: 40,
        ...
        {/* 헤더 */}
        <div style={{ padding: "42px 18px 12px", flexShrink: 0 }}>
          ...
          <div style={{ marginTop: 12, display: "flex", gap: 6, background: C.panel2, border: `1px solid ${C.line}`,
            ...
            {[["people","개인 · 누적"],["fandom","팬덤 · 점유"]].map(([k, lab]) => (
                ...
                return (
                  <div key={fd.f} className="rise" style={{ background: mine ? `${f.color}1A` : C.panel, borderRadius: 16,
                    ...
                    <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 6, fontSize: 11.5, color: C.sub }}>
                      <Crown size={13} color={fd.spots > 0 ? f.color : C.sub} />
        ...
        {/* 내 순위 고정 (개인 모드) */}
        {mode === "people" && me && (
          <div style={{ flexShrink: 0, borderTop: `1px solid ${C.line}`, background: C.frame, padding: "12px 16px 22px" }}>
