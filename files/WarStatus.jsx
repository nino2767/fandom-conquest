...
/* ============================================================
   IP 팬덤 땅따먹기 — S4 점령 전황
   구별 현황(1·2위 격차) · 뒤집힘 / 위태로움 알림
   ============================================================ */

const C = {
  page: "#EBEDF2", frame: "#FFFFFF", panel: "#F4F5F8", panel2: "#EAECF1",
  line: "#E2E5EC", txt: "#1B1D26", sub: "#6C7180", neutral: "#C4CAD8",
};
const FAND = {
...
const myOccupied = D.filter(d => d.leader === MY && d.state === "occupied").length;
const myRisky    = D.filter(d => d.flag === "risky").length;
const myTarget   = D.filter(d => d.second === MY && d.state !== "neutral").length;

export default function WarStatus() {
  const [scope, setScope] = useState("all"); // all | mine
  const [sel, setSel] = useState(null);

  const list = scope === "mine" ? D.filter(d => d.leader === MY || d.second === MY) : D;
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.page, display: "flex", justifyContent: "center",
      ...
      <style>{`
        ...
        @keyframes flamepulse{ 0%,100%{ opacity:.55; } 50%{ opacity:1; } }
      ...
      <div style={{ width: 390, maxWidth: "100%", height: 820, background: C.frame, borderRadius: 40,
        ...
        {/* 헤더 */}
        <div style={{ padding: "42px 18px 10px", flexShrink: 0 }}>
...
function Summary({ icon, n, label, color }) {
  return (
